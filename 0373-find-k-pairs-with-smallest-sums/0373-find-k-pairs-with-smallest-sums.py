...
#             val, num1, num2 i , j = heapq.heappop(heap)
#             ans.append([num1, num2])
            
#             if i < len(nums1) -1 and (i + 1, j) not in visited:
#                 heapq.heappush(heap, ((nums1[i + 1] + nums2[j]), nums1[i + 1, nums2[j], i, j] )
#                 visited.add((nums1[i + 1], nums2[j]))
                
#             if j < len(nums2) -1 and (nums1[i], nums2[j + 1]) not in visited:
#                 heapq.heappush(heap, ((nums1[i] + nums2[j + 1]), nums1[1]nums2[j]
